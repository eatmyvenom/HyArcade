exports.help = `\`/arcadehelp [topic]\`
Get info on how to use commands and what they do. 
Using this with another argument for a command will give more data on how to use the command`;

exports.verify = `\`/verify [player]\`
Link your discord to your account
...
exports.newacc = `\`/addaccount [accounts]\`
Add account(s) by name or uuid to the database. This should not be used for just random people you find, if you do that and add people who will just end up slowing down the whole system you will be blacklisted from using the bot.`;

exports.stats = `\`/stats [player] [game]\`
Give the general arcade stats of a player. If the arcade game argument is filled then it also adds stats from that game. If something looks wrong thats probably because it is, the hypixel api isn't exactly known for providing accurate data. I apologize if this is the case but there is not much at all that can be done about it.`;
...
exports.searching = `The bot searches for a player by going down a list of possiblilites for who a player can be it goes through each of these until one of them returns a valid account
