...
async function main() {
  ...
  Logger.out(JSON.stringify(await c.getLeaderboard("partyGames.wins", false, 2), null, 2));
  Logger.out("");
  await Sleep(5000);
  ...
  Logger.out(JSON.stringify(await c.getAccount("vnmm"), [ "wins", "name", "uuid"]));
  Logger.out("");
  await Sleep(5000);

  Logger.info("vnmm acc - uuid");
  Logger.out(JSON.stringify(await c.getAccount("92a5199614ac4bd181d1f3c951fb719f"), [ "wins", "name", "uuid"]));
  Logger.out("");
  await Sleep(5000);

  Logger.info("vnmm acc - discord");
  Logger.out(JSON.stringify(await c.getAccount("156952208045375488"), [ "wins", "name", "uuid"]));
  Logger.out("");
