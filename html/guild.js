...
async function load () {
  ...
  case "arc": {
    ...
    const agxpD = document.createElement("div");
    ...
    const gxpD = document.createElement("div");
...
async function handleLifetimes (accdata) {
  ...
  for(const e of elements) {
    if(e.hasAttribute("extras")) {
      e.innerHTML =
                `<h2>${ 
                  e.getAttribute("title").replace(/&/g, "&amp;")
                    .replace(/>/g, "&gt;")
                    .replace(/</g, "&lt;")
                    .replace(/"/g, "&quot;") 
                }</h2>${ 
      ...
      e.innerHTML =
                `<h2>${ 
                  ...
                  formatData(
                    ...
                    (pos, acc) => formatLine(pos, acc.name, acc[e.id], acc.uuid)
                  )}`;
    }
  }
}
...
function formatData (accounts, sorter, printer) {
  ...
  accounts.reverse();
  const len = Math.min(accounts.length, maxLength);
...
function formatTimed (accounts, oldAccounts, subtracter, sorter, printer) {
  ...
  for(let i = 0; i < oldAccounts.length; i += 1) {
    ...
    const acc = accounts.find((a) => oldAcc.uuid == a.uuid);
...
function formatLine (pos, name, value) {
  const longName = (`${pos}) ${name}                         `).slice(0, 21);
  if(value > 0) {
