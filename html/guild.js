...
async function refresh() {
    ...
    let servertime = await fetch("http://eatmyvenom.me/share/partygames/timeupdate", {cache : "no-store"});
    ...
    let accdata = await fetch("http://eatmyvenom.me/share/guild.json", { cache: "no-store" });
...
async function handleTimed(timetype, accdata) {
    let accold = await fetch(`http://eatmyvenom.me/share/guild.${timetype}.json`, { cache: "no-store" });
    accdata = JSON.parse(accdata);
